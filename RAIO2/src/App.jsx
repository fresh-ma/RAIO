...
function ProtectedRoute({ children }) {
  const { user, maasApiKey, loading } = useAuth();
  if (loading) return <div className="flex items-center justify-center h-screen bg-sv-bg">
    <p className="pixel-title text-lg">加载中...</p>
  </div>;
  if (!user || !maasApiKey) return <Navigate to="/login" />;
  return children;
}

export default function App() {
  const { user, maasApiKey } = useAuth();
  
  return (
    <Routes>
      <Route path="/login" element={user && maasApiKey ? <Navigate to="/" /> : <LoginPage />} />
